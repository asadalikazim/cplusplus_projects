...
int check_row(int sudoku[9][9], int row, int col)
{
	for (int i = 0; i < 9; i++)
	{
		if (i != col)
		{
			if (sudoku[row][col] == sudoku[row][i]){return -1;}
		}
	}
...
int check_col(int sudoku[9][9], int row, int col)
{
	for (int i = 0; i < 9; i++)
	{
		if (i != row)
		{
			if (sudoku[row][col] == sudoku[i][col]){return -1;}
		}
	}
...
int check_sqr(int sudoku[9][9], int row, int col)
{
	int sqr_row = (row/3)*3;
	int sqr_col = (col/3)*3;
	...
	for (int i = 0; i < 3; i++)
	{
		for (int j = 0; j < 3; j++)
		{
			if (row != i && col != j)
			{
				if (sudoku[row][col] == sudoku[sqr_row + i][sqr_col + j]){return -1;}
			}
		}
	}
   ...
   * *****/
   ...
   for (int row = 0; row < 9; row++)
   {
	   for (int col = 0; col < 9; col++)
	   {
		   if (sudoku[row][col] != 0)
		   {
			   if (check_row(sudoku, row, col) == -1){return -1;}
			   if (check_col(sudoku, row, col) == -1){return -1;}
			   if (check_sqr(sudoku, row, col) == -1){return -1;}
...
void solve_sudoku(int sudoku[9][9], int depth)
{
   ...
   *****/  
   ...
   int row, col, try = -1, checker, next_row, next_col;
	
	//finding the next empty box to solve
	for (row = 0; row < 9; row++)
	{
		for (col = 0; col < 9; col++)
		{
	...
	if (row != 9 && col != 9)
	{
	    //finding the one after that
	    for (next_row = 0; next_row < 9; next_row++)
	    {
		    for (next_col = 0; next_col < 9; next_col++)
		    {
			    if (sudoku[next_row][next_col] == 0 && (next_row != row || next_col != col)){break;}
		    }
		    if (next_col != 9){break;}
	    }
	
	    //try finding its possible value
	    for (try = 1; try < 10; try++)
	    {
		    sudoku[row][col] = try;
		
		    //the tried value is valid
		    checker = check_sudoku(sudoku);
		    if (checker == -1)
		    {
			    //solve the rest of the puzzle with this value
			    solve_sudoku(sudoku, depth);
		    }
			
			//rest of the puzzle solved... so this value is correct... keep it
		...
		print_sudoku(sudoku);
		//if all tries failed... set box back to zero
		if (try == 10){sudoku[row][col] = 0;}
	}
